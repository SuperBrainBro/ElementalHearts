		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
		{
			...
			tasks.Insert(ShiniesIndex + 1, new PassLegacy("ElementalHeartsBiomeGen", delegate (GenerationProgress progress)
			{
				...
				for (int i = 0; i < (int)Main.maxTilesX / 500; i++)
				{
					...
					{
						WorldGen.TileRunner(XvalueMid, YvalueMid, (double)WorldGen.genRand.Next(50, 100), 1, TileType<CrystalGrassPink>(), false, 0f, 0f, true, true); 
					}
					int x;
					int y;
					int maxTries = 50000;
